...
function createProjectSpec(opts, resourceDir, templateInfo)
{
    ...
    for each (var doc in docsMap) {
        ...
        /* In TI-RTOS, docs are in a different path when building the products,
         * as opposed to the released product. Therefore, we can't check for the
         * existence of the file at the build time on a specific path. Currently,
         * we'll assume the docs are in the top directory, but that's a hack that
        ...
        for (var i = 0; i < docsPaths.length; i++) {
            var docFile = opts.productDir + docsPaths[i] + "/" + doc.filePath;
                        ...
                        for each (var group in template.groups) {
                            if (group == element.id) {
                                ...
                                for (var i = 0; i < fArr.length; i++){
                                    var tmpArr = fArr[i].deviceId.split('|');
                                    for (var j = 0; j < tmpArr.length; j++) {
                                        deviceArr.push(tmpArr[j]);
                                    }
                                }
...
/*
 *  @(#) xdc.tools.product.trexgen; 1, 0, 0,68; 12-8-2015 17:33:51; /db/ztree/library/trees/xdctools/xdctools-g03x/src/
 */
